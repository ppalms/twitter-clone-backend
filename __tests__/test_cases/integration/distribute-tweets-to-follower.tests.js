...
describe('Given user A and user B', () => {
  ...
  beforeAll(async () => {
    ...
    userAsTweet1 = await when.we_invoke_tweet(
      userA.username,
      chance.paragraph()
    );
    userAsTweet2 = await when.we_invoke_tweet(
      userA.username,
      chance.paragraph()
    );
  });
  ...
  describe('When user B follows user A', () => {
    ...
    it("Adds user A's tweet to user B's timeline", async () => {
      await then.tweet_exists_in_TimelinesTable(
        userB.username,
        userAsTweet1.id
      );
      await then.tweet_exists_in_TimelinesTable(
    ...
    describe('When user B unfollows user A', () => {
      ...
      it("Removes user A's tweet from user B's timeline", async () => {
        await then.tweet_does_not_exist_in_TimelinesTable(
          userB.username,
          userAsTweet1.id
        );
        await then.tweet_does_not_exist_in_TimelinesTable(
